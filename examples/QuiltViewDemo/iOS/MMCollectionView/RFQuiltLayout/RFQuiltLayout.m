...
//    CGRect scrollFrame = CGRectMake(self.collectionView.frame.origin.x, self.collectionView.frame.origin.y, self.collectionView.frame.size.width, self.collectionView.frame.size.height);
...
// item像素,item大小的基数,用来乘以blockSize得到最后的frameSize
...
- (CGRect) frameForIndexPath:(NSIndexPath*)path {
    ...
    // 内切矩形,collctionView的content
    CGRect contentRect = UIEdgeInsetsInsetRect(self.collectionView.frame, self.collectionView.contentInset);
    
    if (isVert) {
        // CGRectGetWidth(contentRect) / self.blockPixels.width
